  ...
  get(attributeName) {
    if (this.#attributes.hasOwnProperty(attributeName)) {
      return {
        ...
        setText: (value) => {
          this.#attributes[attributeName].text = value;
        }, // Set text
        getTagId: () => this.#attributes[attributeName].tagId, // Get tagId
        setTagId: (tagId) => {
          this.#attributes[attributeName].tagId = tagId;
        }, // Set tagId
      };
    } else {
 ...
 */
function createLog(lstPerson) {
  const linea1 = "-----";
  const linea2 = "---";
  const linea3 = IntegranteGetString(lstPerson);
 ...
 */
function IntegranteGetString(param) {
  let stg = "";
...
function getIntegrantes() {
  ...
  for (let rows = 0; rows < integrantes.length; rows++) {
    const _name1 = {
      ...
      tagId: integrantes[rows]
        .getElementsByTagName("dd")[0]
        .getAttributeNode("id").textContent, // Attribute ID
    };
    ...
    const _lastname2 = {
      ...
      tagId: integrantes[rows]
        .getElementsByTagName("dd")[3]
        .getAttributeNode("id").textContent, // Attribute ID
    };

    lista_integrantes.push(
      new Person({
        name1: _name1,
        name2: _name2,
...
function compareNames(person1, person2) {
  ...
  if (
    !compareText(
      getListTextTagID(person1, NAME1, NAME2),
...
function getListTextTagID(person, name1, name2) {
  const lstTextTagID = [
    [person.get(name1).getText(), person.get(name2).getText()],
...
function createNewIntegrante(values) {
  const integrantes = document.getElementById("integrantes").getElementsByTagName("dl");
  const rows = contadorIntegrante == 1 ? 0 : 1; // contadorIntegrante - 1

  integrantes[rows].getElementsByTagName("dd")[0].innerText = values.name1;
  integrantes[rows].getElementsByTagName("dd")[1].innerText = values.name2;
  integrantes[rows].getElementsByTagName("dd")[2].innerText = values.lastname1;
  integrantes[rows].getElementsByTagName("dd")[3].innerText = values.lastname2;
}
